...
cpdef double extraterr(double gsc, double lat, double n, double h, double Eo, double sunrise_h, double glob_h):
...
cpdef np.ndarray[double] apply_extraterr(double gsc, double lat, np.ndarray col_n, np.ndarray col_h, np.ndarray col_Eo, np.ndarray col_sunrise_h, np.ndarray col_glob_h):
    cdef Py_ssize_t i, n = len(col_h)
    cdef np.ndarray[double] res = np.empty(n)
    for i in range(len(col_h)):
        res[i] = extraterr(gsc, lat, col_n[i], col_h[i], col_Eo[i], col_sunrise_h[i], col_glob_h[i])
        # res[i] = extraterr(gsc, coslat, sinlat, col_h[i], col_Eo[i], col_ws[i], col_sunrise_h[i], col_cosdec[i], col_sindec[i], col_glob_h[i])
    return res
